...
def recursion_binarysearchvalues(L, v):
    ...
    return binary(L, v, [], 0, len(L)-1)


def binary(values, ans, ans1, first, last):
    bottom = (first+last) // 2
    ans1.append((bottom, values[bottom]))
    if values[bottom] > ans and first < last:
        return binary(values, ans, ans1, first, bottom - 1)
    elif values[bottom] < ans and first < last:
        return binary(values, ans, ans1, bottom + 1, last)
    return ans1
